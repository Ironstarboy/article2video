...
class InfiniteTalkPipeline:
    ...
    def generate_infinitetalk(self,
        ...
        def _encode_on(device):
            """Run the text encoder on `device` and return (context, context_null)."""
            self.text_encoder.model.to(device)
            ctx, ctx_null = self.text_encoder([input_prompt, n_prompt], device)
            return ctx.to(self.device), ctx_null.to(self.device)

        if os.environ.get('IT_T5_ON_CPU'):
            # Force CPU: see the NaN note below.
            self.text_encoder.model.to('cpu')
            context, context_null = self.text_encoder(
                [input_prompt, n_prompt], torch.device('cpu'))
            context = context.to(self.device)
            context_null = context_null.to(self.device)
        elif not self.t5_cpu:
            context, context_null = _encode_on(self.device)
            # Guard: on the PPU the quanto fp8 weight matmul can return all-NaN text
            # embeddings. That NaN silently poisons the text-conditioned DiT pass and the
            # whole video comes out black, so detect it and fall back to CPU encoding
            # rather than emitting a broken video.
            if torch.isnan(context.float()).any() or torch.isnan(context_null.float()).any():
                logging.warning(
                    'T5 text encoder produced NaN on %s; falling back to CPU encoding '
                    '(set IT_T5_ON_CPU=1 to skip the failed GPU attempt).', self.device)
                context, context_null = _encode_on(torch.device('cpu'))
            elif offload_model:
        ...
        # === TEMP NaN DIAGNOSTIC ===
        if os.environ.get('IT_DEBUG_NAN'):
            for _nm, _c in (('context', context), ('context_null', context_null)):
                for _i, _t in enumerate(_c):
                    _f = _t.float()
                    print(f'[diag-text] {_nm}[{_i}] shape={tuple(_t.shape)} dtype={_t.dtype} '
                          f'nan={bool(torch.isnan(_f).any())} device={_t.device}', flush=True)
        # === END TEMP ===
        ...
        while True:
            ...
            with torch.no_grad(), no_sync():
                ...
                for i in progress_wrap(range(len(timesteps)-1)):
                    ...
                    # NOTE: torch_gc() (empty_cache + ipc_collect) is intentionally NOT
                    # called between these forwards. It flushed the caching allocator
                    # after every one of the 3 CFG passes, so each pass had to re-allocate
                    # its working memory from scratch — measured at ~231s per denoising
                    # step here. Reusing the allocator cache is safe: peak usage is well
                    # under the 96GB card budget. The chunk-level gc above/below remains.
                    noise_pred_drop_audio = noise_pred_drop_text = noise_pred_uncond = None
                    ...
                    # === TEMP NaN DIAGNOSTIC ===
                    if os.environ.get('IT_DEBUG_NAN') and i < 3:
                        if i == 0:
                            for _n, _t in (('audio_embs', audio_embs), ('latent_in', latent),
                                           ('context', context[0] if isinstance(context, (list, tuple)) else context),
                                           ('clip_context', clip_context[:, :1])):
                                _f = _t.float()
                                print(f'[diag-in] {_n:12s} nan={bool(torch.isnan(_f).any())} '
                                      f'inf={bool(torch.isinf(_f).any())} '
                                      f'absmax={_f.nan_to_num(0).abs().max().item():.4g}', flush=True)
                        _dbg = {'cond': noise_pred_cond, 'merged': noise_pred, 'latent': latent,
                                'drop_audio': noise_pred_drop_audio,
                                'drop_text': noise_pred_drop_text, 'uncond': noise_pred_uncond}
                        for _nm, _t in _dbg.items():
                            if _t is None:
                                continue
                            _f = _t.float()
                            print(f'[diag] step{i} {_nm:10s} nan={bool(torch.isnan(_f).any())} '
                                  f'inf={bool(torch.isinf(_f).any())} '
                                  f'absmax={_f.nan_to_num(0).abs().max().item():.4g}', flush=True)
                    # === END TEMP ===
