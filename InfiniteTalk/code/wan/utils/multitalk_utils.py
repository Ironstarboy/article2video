...
# torch.compile generates a Triton kernel here, and the PPU (T-HEAD ZW810E) driver
# rejects the generated launcher with "CUDA driver error: invalid argument" while
# loading the kernel. Run it eagerly instead: it is called once per denoising step,
# not once per layer, so the cost is small next to a working pipeline.
