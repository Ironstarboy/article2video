...
- **数字人出镜(可选)** —— 成片里叠一个数字人朗读当前帧台词,四角任选(默认右上)、大小可手输(默认 300×300),也能一键切成**纯 PPT 版**;还能勾「**只保留人像(背景透明)**」把背景抠掉、人像贴画面下缘,幻灯片内容不再被那块不透明卡片压住。
...
任务是磁盘持久化的状态机:`uploaded → analyzing → analyzed → building → preview → rendering → rendered`;播报视频走支线状态 `avatar_building`,成功或失败都回到进入前的状态,**失败不阻塞出片**。架构细节、关键设计决策与完整接口清单见 [`ARCHITECTURE.md`](ARCHITECTURE.md)。
...
3. **开始分析**:调用 DeepSeek 网关。讲解视频为多轮调用(30 分钟档约 6-8 次),耗时 5-15 分钟属正常,页面显示阶段进度。
4. **数字人播报视频(可选)**:分析完成后即可一键生成,不需先构建、也不等成片;按「合成配音 → 生成片段 → 拼接」三步展示进度与预计剩余,完成后独立播放、下载。
5. **构建预览**:多实例并行合成配音;旁白量不足目标时自动二次拓展内容;组装 HyperFrames 工程并同步拉起 Studio 编辑器。
6. **渲染成片**:1080p 高清渲染(约 5-20 分钟,视时长),完成后就地播放与下载;渲染结束自动叠加上数字人(若已开启)。
7. 成片或在预览页随时可改的数字人几何(出镜开关 / 角落 / 大小 / **是否抠像**)改完需**重新构建预览、重新渲染**才生效。
8. **随时回来**:每一步(分析 / 构建 / 渲染 / 播报)完成或失败都会记进项目历史;从入口页点某一步即回到对应结果,正在进行的任务在列表上就能看到进度。
...
| `TTV_AVATAR_ADDR` / `_IMAGE` / `_SIZE` / `_CORNER` | `127.0.0.1:50051` / `assets/avatars/jinli.png` / `300` / `tr` | 数字人服务、形象、边长、默认角落 |
| `TTV_AVATAR_CUTOUT` | `0` | `1` = 全局默认开启「只保留人像(背景透明)」(也可按任务勾) |
...
日志统一收在 `logs/`:`backend.log`(后端)、`tts/*.log`(配音实例)、`studio/<job_id>.log`(每任务 Studio 输出)。
...
├── web/index.html           单文件 SPA(项目历史入口页 + 创作页 + 预览页,Studio 为唯一预览入口)
├── deploy/                  setup / 启动脚本 / nginx 路由 / 校验脚本
├── styles/                  三套经典风格设计脚本
├── proto/                   数字人 gRPC 契约
├── docs/adr/                架构决策记录
├── assets/                  品牌 Logo、数字人形象、字体/BGM/GSAP(后三者不入库)
...
| 方法 | 路径 | 说明 |
|---|---|---|
| `GET` | `/api/styles` | 四维度选项 + 预设 + 数字人尺寸档位与抠像默认值 |
...
- **数字人几何改动不追溯**:换大小 / 换角落 / 切「不出镜」/ 开关抠像后需重新构建预览再渲染才生效(片段与缓存保留,切回来很快)。
...
| [`CHANGELOG.md`](CHANGELOG.md) | 版本与变更记录(当前 v2.5) |
