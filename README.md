...
- **本地 CosyVoice3 单帧配音时长波动很大**(实测同一句 3 次:4.08s / 20.00s / 20.00s;历史任务里也存在 47 字出 0.37s、36 字出 23.9s 的极端帧)。成片与播报视频的节奏都直接受它影响(两者同源、同一份配音),需要在 TTS 服务侧单独排查,与本仓库的拼接/叠加逻辑无关
