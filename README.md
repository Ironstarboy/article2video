...
- **本地 CosyVoice3 解码不稳定**(实测同一句会给出 0.04s–36.8s:残句或复读;官方示例在本机同样复现)。已加两层兜底:合成后**按时长验收**(实际/预期 = 字数÷4.2,窗口 0.65–1.8,不通过就重采,最多 6 次取最接近的一次)+ 客户端再兜一次。实测同一任务 11 个台词帧从 5/11 合格提升到 **11/11**;代价是坏样本要重采(TTS 阶段约 1.5 倍耗时)。根因在本机 TTS 运行时/权重,彻底解决需重装运行时或换引擎(豆包云 / Qwen3-TTS 目前都没有可用凭据与权重)
