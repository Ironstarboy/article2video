...
5. **构建预览**:本地 CosyVoice3 **三实例(GPU1/2/3)并行合成**配音(自然语速)+ 旁白量不足目标时自动二次拓展内容 + 组装 HyperFrames 项目;预览就绪时 **Studio 编辑器已同步启动**,页面直接进入完整编辑器(时间线、逐帧编辑、实时预览)
6. **渲染成片**:1080p 高清渲染(约 5-20 分钟,视时长;30 分钟讲解片渲染更久),完成后同位置切换为成片播放,在线观看并下载(按钮显示格式 + 文件大小)
