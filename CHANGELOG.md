...
- **测试**:后端 `smoke_test.py` 新增 15 项(偏好出厂默认/读写/损坏与类型错自愈/只认布尔/接口 200-400-空体/`/api/styles` 下发/偏好只影响没存过几何的任务),并把 3 条编码了旧默认(true→false)的既有断言改成跟随口径 —— 全套 **319 项通过**;前端 `tests/avatar-page.test.js` 新增 11 项(开关回显与文案、POST 只发一个布尔字段、失败提示不被刷新冲掉、失败时开关回退、拿不到偏好时置灰不误显示为关、创作页勾选框跟随偏好)—— **38 项通过**(顺带修了测试自身一个坑:上一个用例把 `fetch` 换成"永远抛错"却没恢复,后续用例全废)
